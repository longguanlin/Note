...
未安装**docker**的可以参考 [Linux安装docker](https://blog.csdn.net/weixin_43792363/article/details/130182744?spm=1001.2014.3001.5501)

### 2、安装mysql

nacos默认采用内嵌式数据库 **Derby** ，数据存放在 /nacos/data/derby-data 目录下，我们配置为**mysql**，未安装**mysql**的可以参考 [Docker安装mysql](https://blog.csdn.net/weixin_43792363/article/details/130183475?spm=1001.2014.3001.5501)

### 3、Mysql建库建表

安装docker会配置数据持久化将nacos的数据保存到mysql新建的数据库中
...
*nacos是使用springboot写的，这的custom.properties相当于application.properties的子配置*
...
5、nacos容器就安装成功了可以打开浏览器进行登录：

https:xx.xx.xx.xx:8848/nacos 账号：nacos 密码：nacos

![nacos-server](C:\Users\15911\AppData\Roaming\Typora\typora-user-images\image-20230508231949828.png)
...
change master to
    ...
    master_password='P@ssw0rd',					## 同步账号密码
